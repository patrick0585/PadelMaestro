...
describe("buildSeasonTrend", () => {
  ...
  it("emits null for a player who has not yet appeared by a given day", async () => {
    ...
    // All four players have appeared by Day B, so values are non-null.
    expect(byName.Thomas[1] as number).toBeLessThan(byName.Michi[1] as number);
    expect(byName.Patrick[1] as number).toBeLessThan(byName.Thomas[1] as number);
    ...
    expect(byName.Patrick[0] as number).toBeLessThan(byName.Michi[0] as number);
    expect(byName.Patrick[0] as number).toBeLessThan(byName.Thomas[0] as number);
  });
